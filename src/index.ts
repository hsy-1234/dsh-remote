 ...
 *     install / login) — loaded as its own loader entry (lib/remote.js, the
 *     same pattern the shipped @Remote services use),
...
/** Structural view of the dshRemote service provided by the lib/remote.js entry. */
interface DshRemoteServiceLike {
  describeStatus(): Promise<string>
  ensureConfig(): Promise<{ ok: boolean; message: string; needsRestart?: boolean }>
}
...
export function apply(ctx: Context): void {
  console.log('[dsh-remote] apply start')

  // 1. Old-browser compatibility: crypto.randomUUID polyfill
  ...
  if (webServer !== undefined && typeof webServer.tapIndex === 'function') {
    ...
    console.log('[dsh-remote] polyfill tap registered')
  }

  // 2. Slash commands (also available without the browser bundle)
  ...
  if (commands !== undefined) {
    commands.register({
      ...
      handler: async () => {
        const svc = ctx.get('dshRemote') as DshRemoteServiceLike | undefined
    ...
    console.log('[dsh-remote] commands registered')
  }
}
